...
public class MeshUtil {
	public static void render(int[] data) {
		...
		synchronized(gl) {
			...
			gl.glVertexAttribPointer(0, 3, GL.GL_FLOAT, false, 11 * 4, 0 * 4);
			...
			gl.glDrawArrays(GL2ES3.GL_QUADS, 0, data[1]);
	...
	public static int[] load(float[] data) {
		...
		synchronized(gl) {
			...
			buf.put(data);
			buf.flip();
			gl.glBufferData(GL.GL_ARRAY_BUFFER, 4 * data.length, buf, GL.GL_STATIC_DRAW);
			gl.glBindBuffer(GL.GL_ARRAY_BUFFER, 0);
